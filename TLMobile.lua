...
secGame.Font = Enum.Font.GothamBold; secGame.TextSize = 11
secGame.TextColor3 = _C3_WHITE
...
secProf.Font = Enum.Font.GothamBold; secProf.TextSize = 11
secProf.TextColor3 = _C3_WHITE
...
srvCard.Size = UDim2.new(1, -PAD*2, 0, 140)
...
local function GetAimPart(character)
    ...
    if AimbotConfig.AimPart == "Smart" or AimbotConfig.AimPart == "Head" then
...
local function GetTargetPosition(player, targetPart, dt)
    ...
    if rootPart then
        if not AimbotCache[player] then
            AimbotCache[player] = { 
            ...
            local cache = AimbotCache[player]
...
local function GetBestTarget()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        ...
        if worldDist > AimbotConfig.AutoAimDistance then continue end
        ...
        if distFromCenter > AimbotConfig.FOV then continue end
        
        -- MASTER HYBRID SCORING:
        -- Prioritize players near crosshair AND near user
...
local function SmoothAim(targetPosition, dt)
    ...
    -- Master Ultra-Stable Smoothing
    local s = math.clamp(AimbotConfig.Smoothness, 0.0, 0.99)
    if s == 0 then
        Camera.CFrame = CFrame.new(currentCFrame.Position) * targetRotation
        return
    end
...
local function PerformAimbot(dt)
    ...
    if AimbotConfig.CurrentTarget then
        ...
        if player and player.Parent and IsPlayerAlive(player) then
            local char = GetCharacter(player)
            local part = GetAimPart(char)
            
            -- Immediate visibility and range re-check
            if part and (not AimbotConfig.WallCheck or IsVisible(part)) then
                local screenPos, onScreen = Camera:WorldToViewportPoint(part.Position)
                local distFromCenter = (Vector2.new(screenPos.X, screenPos.Y) - screenCenter).Magnitude
                
                -- Dynamic lock leniency: 2x FOV for existing target
                if onScreen and distFromCenter <= AimbotConfig.FOV * 2.0 then 
                    target = {
                        Player = player,
                        Part = part,
    ...
    if not target then
        ...
        if target then
            ...
            AimbotConfig.LastTargetTime = os.clock()
        end
    end
    ...
    if target and target.Part then
        local targetPosition = GetTargetPosition(target.Player, target.Part, dt)
        
        if targetPosition then
            -- Visuals: Target Line
            if AimbotConfig.ShowTargetLine then
            ...
            -- Execute Aimlock
            if AimbotConfig.Aimlock or AimbotConfig.AimlockToggle then
                local isAiming = AimbotConfig.AimlockToggle or UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) or AimbotConfig.AutoFire
                if isAiming then
                    SmoothAim(targetPosition, dt)
                end
            end
            ...
            if AimbotConfig.AutoFire and not AimbotConfig.TriggerBot then
                local screenPos, onScreen = Camera:WorldToViewportPoint(targetPosition)
                if onScreen then
                    local distToTarget = (Vector2.new(screenPos.X, screenPos.Y) - UserInputService:GetMouseLocation()).Magnitude
...
local function StartAimbot()
    ...
    -- Use BindToRenderStep with high priority to run AFTER the game's camera updates
    -- This is crucial to prevent jitter when the player is moving
    RunService:BindToRenderStep("TLAimbotMaster", Enum.RenderPriority.Camera.Value + 1, function(dt)
    ...
    AimbotConnection = true
end

local function StopAimbot()
    if AimbotConnection then
        RunService:UnbindFromRenderStep("TLAimbotMaster")
...
LocalPlayer.CharacterRemoving:Connect(function()
    ...
    -- StopTriggerBot() 
    pcall(function() if FOVCircle.Remove then FOVCircle:Remove() elseif FOVCircle.Destroy then FOVCircle:Destroy() end end)
    pcall(function() if TargetLine.Remove then TargetLine:Remove() elseif TargetLine.Destroy then TargetLine:Destroy() end end)
end)
...
local nnContainer, nnContent, nnAddRow, _nnFolderApi = makeMiscGamePanel(nnColor, 4)
...
local stopDoggy        = _qag._TL_stopDoggy        or _qag_env._TL_stopDoggy
